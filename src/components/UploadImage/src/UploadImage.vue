...
export default {
  ...
  methods: {
    ...
    isAllowedUpdate(type) {
      ...
      if (type === 'multiple') {
        if (this.multiple && this.photoFiles.length > this.uploadCountLimit) {
          return true;
        }

        if (
          this.multiple
          && this.photoFiles.length + this.attachments.length
    ...
    updateFiles(base64String, fileName, fileSize) {
    ...
    handlePreview() {
      ...
      this.photoFiles.forEach((ele) => {
        ...
        reader.onload = (e) => {
          ...
          if (useCompress) {
            self.compressFile(targetResult, w, h, ele, self.updateFiles);
          } else {
            self.updateFiles(targetResult, ele.name, ele.size);
          }
        };
      });
    },
    ...
    compressFile(imgBase64Data, maxWidth, maxHeight, file, updateFiles) {
      ...
      img.onload = () => {
        ...
        updateFiles(base64String, file.name, file.size);
      };
    },
  },
};
